...
export function TrafoCard({ trafoStatus, onNameChange, threshold }: TrafoCardProps) {
  ...
  useEffect(() => {
    setDraft(trafoStatus.config.name)
  }, [trafoStatus.config.name])
  ...
  const rPhase = phases.find(p => p.phase === 'R')!
  const sPhase = phases.find(p => p.phase === 'S')!
  const tPhase = phases.find(p => p.phase === 'T')!
  ...
  return (
    <div className={`rounded-xl border-2 p-4 flex flex-col gap-3 ${borderCls} transition-colors`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`w-2.5 h-2.5 rounded-full ${
            overallStatus === 'Warning' ? 'bg-yellow-500 animate-pulse' : 'bg-green-500'
          }`} />
          {editing ? (
            <input
              ref={inputRef}
      ...
      <div className="flex flex-col items-center gap-2">
        <PhaseGauge phase="R" value={rPhase.value} baseline={rPhase.baseline}
          status={rPhase.status} threshold={threshold.warning} size={88} />
        <div className="flex justify-between w-full">
          <PhaseGauge phase="S" value={sPhase.value} baseline={sPhase.baseline}
            status={sPhase.status} threshold={threshold.warning} size={88} />
          <PhaseGauge phase="T" value={tPhase.value} baseline={tPhase.baseline}
            status={tPhase.status} threshold={threshold.warning} size={88} />
        </div>
      </div>
